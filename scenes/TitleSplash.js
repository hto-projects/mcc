export default class TitleSplash extends Phaser.Scene {
  ...
  create() {
    ...
    const pressStartText = this.add.bitmapText(width * .5, height * .75, 'pixelfontyellow', 'PRESS START', 24).setOrigin(0.5);

    const blinkEvent = this.time.addEvent({
      delay: 500,
      loop: true,
      callback: () => {
        pressStartText.text = pressStartText.text === "" ? 'PRESS START' : "";
      }
    });
