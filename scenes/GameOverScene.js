...
const saveNewHighScore = (newHighScore) => {
  const highScores = getHighScores();
  highScores.push(newHighScore);
  setHighScores(highScores);
}

const getSortedHighScoresWithNew = (newScore) => {
  const highScores = getHighScores();
...
export default class GameOverScene extends Phaser.Scene {
  ...
  create() {
    ...
    const instructionsText = this.add.bitmapText(width * .5, height * .2, 'pixelfont', "Enter your initials and press START to save your score.", 10).setOrigin(0.5);
    ...
    if (!this.win) {
      ...
      instructionsText.setText("Press START to return to the title screen.");
